...
def main():
    ...
    VALORES_LQR = {
        ...
        "b_param": 1.0,   
        "trim": 43,      
        "ki_ang": 0    # <--- NUEVO: Ganancia Integral
    }
    # ===============================================

    radio = RF24(CE_PIN, CSN_DEV)
    print("=== Horus GCS: Control ÓPTIMO LQI ===")
    ...
    def lqr_set(which, val):
        # AÑADIDO ki_ang CON EL SELECTOR 6
        sel = {"q_ang":0, "q_rate":1, "r_ctrl":2, "sp":3, "b_param":4, "trim":5, "ki_ang":6}[which]
        q = q88_from_float(val)
        send(CMD_SET_PID, mm=sel, mus=q)
        VALORES_LQR[which] = val
    ...
    def print_status():
        ...
        print(f"[STATUS] age={age}ms | Roll_Estimado: {roll_filtrado:+.1f}° | Gx_f: {telem['gx']/10.0:+.1f}°/s | Cal: {cal} Arm: {arm}")
    ...
    print("\n⚙️ Enviando configuración inicial al dron...")
    lqr_set("q_ang", VALORES_LQR["q_ang"])
    time.sleep(0.05)
    lqr_set("q_rate", VALORES_LQR["q_rate"])
    time.sleep(0.05)
    lqr_set("r_ctrl", VALORES_LQR["r_ctrl"])
    time.sleep(0.05)
    lqr_set("b_param", VALORES_LQR["b_param"])
    time.sleep(0.05)
    lqr_set("trim", VALORES_LQR["trim"])
    time.sleep(0.05)
    lqr_set("ki_ang", VALORES_LQR["ki_ang"])
    time.sleep(0.05)
    print("✅ Dron configurado con LQI y listo para despegar.\n")

    # --- HILO DEL MENÚ CLI ---
    def cli_thread():
        nonlocal show_graph
        print("Comandos Vuelo: status | cal | arm | dis | stop | thr <us>")
        print("Comandos LQI:   lqr q_ang <f> | lqr q_rate <f> | lqr r_ctrl <f> | lqr b_param <f> | lqr trim <f> | lqr ki_ang <f>")
            ...
            while not stop_evt.is_set():
                ...
                if s.startswith("lqr "):
                    ...
                    if len(parts) == 2 and parts[1] == "show":
                        print("Valores LQI Actuales:", VALORES_LQR)
                        continue
                    if len(parts) != 3 or parts[1] not in ("q_ang", "q_rate", "r_ctrl", "b_param", "trim", "ki_ang"):
                        print("Uso: lqr q_ang/q_rate/r_ctrl/b_param/trim/ki_ang <float>")
                        continue
                    try: lqr_set(parts[1], float(parts[2]))
                    except ValueError: pass
                    continue

                if s.startswith("sp "):
                    try: lqr_set("sp", float(s.split()[1]))
    ...
    if GRAPHING_ENABLED:
        plt.ion()
        fig, (ax_ang, ax_rate) = plt.subplots(2, 1, figsize=(8, 6))
        fig.canvas.manager.set_window_title('Horus Telemetry - LQI Tuning')
        
        line_roll, = ax_ang.plot([], [], label='Ángulo Roll (°)', color='blue')
        line_sp, = ax_ang.plot([], [], label='Setpoint (°)', color='red', linestyle='--')
        ax_ang.set_ylim(-45, 45)
        ax_ang.set_xlim(0, 200)
        ax_ang.set_ylabel('Grados')
        ax_ang.legend(loc='upper right')
        ax_ang.grid(True)

        line_gx, = ax_rate.plot([], [], label='Velocidad Gx (°/s)', color='green')
        ax_rate.set_ylim(-150, 150) 
        ax_rate.set_xlim(0, 200)
        ax_rate.set_ylabel('Grados / Segundo')
        ...
        while not stop_evt.is_set():
            if show_graph:
                ...
                sp_history.append(VALORES_LQR["sp"])
                
                telem_now, _ = link.snapshot()
                if telem_now:
                    gx_history.append(telem_now['gx'] / 10.0)
                else:
                    gx_history.append(0.0)

                line_roll.set_data(range(len(roll_history)), list(roll_history))
                line_sp.set_data(range(len(sp_history)), list(sp_history))
                line_gx.set_data(range(len(gx_history)), list(gx_history))
                
                if len(roll_history) >= 200:
                    ax_ang.set_xlim(len(roll_history)-200, len(roll_history))
                    ax_rate.set_xlim(len(gx_history)-200, len(gx_history))
