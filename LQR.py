...
# -*- coding: utf-8 -*-
"""
Horus GCS V7 — LQI Óptimo + Anti-Windup Dinámico + Inversión de Fase
Sincronizado con main.c V7 (STM32)

Protocolo LQI:
  CmdPkt   "<BBHHhhhhhhhhHBB" → 26 bytes
  TelemPkt "<BBIhhhhhHHHHHBB" → 28 bytes
"""
...
INVERT_ROLL_AXIS  = True   # <-- Puesto en True para arreglar tu problema de motores 1 y 3 vs 2 y 4
...
CMD_FMT   = struct.Struct("<BBHHhhhhhhhhHBB")     # 26 bytes
...
# ═══════════════════════════════════════════════════════════════
#  Modelo LQI Físico (Obtenido via System Identification)
# ═══════════════════════════════════════════════════════════════
class DroneLQIModel:
...
class CmdState:
    def __init__(self):
        ...
        self.q_roll  = [10.0, 1.0, 5.0]  
        self.r_roll  = 1.0
        
        self.q_pitch = [10.0, 1.0, 5.0]
        self.r_pitch = 1.0
...
class HorusGCS:
    ...
    def __init__(self):
        ...
        # Inicializar el modelo matemático LQI
    ...
    def _print_status(self):
        ...
        # --- VISUALIZACIÓN DE MOTORES Y DINÁMICAS RESTAURADA ---
        print(f"[{link} {age}ms] Roll={telem['roll']:+6.1f}° SP={self.cmd.sp_roll:+5.1f}° | Pitch={telem['pitch']:+6.1f}° SP={self.cmd.sp_pitch:+5.1f}°")
    ...
    def _cli_thread(self):
        help_text = """
╔══════════════════════════════════════════════════════════╗
║        Horus GCS V7 — Control LQI Basado en Modelo       ║
╠══════════════════════════════════════════════════════════╣
║ Vuelo:                                                   ║
║   cal              calibra IMU (dron nivelado)           ║
║   arm              arma (motores quietos a 1000µs)       ║
║   thr <us>         throttle base 1000..1600              ║
║   start            inicia motores con LQI                ║
║   dis              desarma e ignora integrales           ║
...
║      ej: tune roll 100 10 50 0.001                       ║
║   tune pitch <Q_ang> <Q_rate> <Q_int> <R>                ║
║      ej: tune pitch 100 10 50 0.001                      ║
║   aw <valor>       Ajusta límite Anti-Windup (ej: aw 50) ║
║   status           Ver Q, R, Motores y K actuales        ║
║                                                          ║
║ Otros:                                                   ║
║   imu on|off       habilita/deshabilita IMU              ║
║   quit             cerrar GCS                            ║
╚══════════════════════════════════════════════════════════╝
"""
        print(help_text)
        ...
        while not self.stop_evt.is_set():
            ...
            try:
                if cmd == "status":
                    ...
                    if ok:
                        print("🔒 ARMADO — motores en 1000µs. Integrales a 0.")
                    else:
                        print("❌ Sin ACK (¿failsafe activo? intenta 'cal' primero)")
                elif cmd == "thr" and len(args) == 1:
                    v = int(args[0])
                    if not 1000 <= v <= 1600:
                        print("⚠️  Rango válido: 1000–1600 µs")
                    else:
                        ok = self._send(flags=CMD_SET_THR, thr_us=v)
                        print(f"✅ Throttle → {v}µs" if ok else "❌ Sin ACK")
                elif cmd == "start":
                    ok = self._send(flags=CMD_START_MOTORS)
                    print("🚀 Motores girando con LQI" if ok else "❌ Sin ACK")
                elif cmd == "dis":
    ...
    def _run_graph(self):
        ...
        fig.suptitle("Horus GCS V7 — Roll & Pitch (LQI)",
    ...
    def run(self):
        print("=== Horus GCS V7 - SystemID LQI Óptimo ===")
