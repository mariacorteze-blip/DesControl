...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : STM32 MIMO LQI Control - V7 (LQI + AW Dinámico + Fast PWM 250Hz)
  *
  * Modelo en espacio de estados con integrador (LQI):
  * Estados x = [roll, gx_rate, int_e_roll, pitch, gy_rate, int_e_pitch]^T 
  * Salidas y = [roll, pitch]^T                     
  *
  * Ley LQI:    u_ctrl = -K * x 
  *
  * PROTOCOLO LQI (26 bytes CMD, 28 bytes TELEM):
  * CmdPkt   -> 26 bytes
  * TelemPkt -> 28 bytes
  ******************************************************************************
  */
 ...
 * TEMPORIZACIÓN
 * --------------------------------------------------------------- */
static const uint32_t HB_PERIOD_MS = 20;     // Telemetría a 50 Hz
static const uint32_t FAILSAFE_MS  = 3000;   // Sin cmd = 3 s -> disarm
static const float    dt_control   = 0.005f; // Loop de control a 200 Hz
...
// CORRECCIÓN DE RETRASO: alpha_gyro subido a 0.4 para respuesta casi instantánea
static const float alpha_gyro = 0.4f; 

/* ---------------------------------------------------------------
 * ESTIMADOR (filtro complementario)
 * --------------------------------------------------------------- */
static float roll_deg  = 0.0f;
static float pitch_deg = 0.0f;

/* ---------------------------------------------------------------
 * SETPOINTS, ACUMULADORES Y LEY DE CONTROL LQI
 * --------------------------------------------------------------- */
...
static float int_limit = 50.0f; 

static float K_roll_ang   = 1.3043f;
static float K_roll_rate  = 1.6002f;
static float K_roll_int   = 0.0f;     
static float K_pitch_ang  = 1.2999f;
static float K_pitch_rate = 1.5940f;
static float K_pitch_int  = 0.0f;     
...
static uint16_t base_throttle = 1000;
...
typedef char _check_CmdPkt_size  [sizeof(CmdPkt)   == 26 ? 1 : -1];
...
static void control_update(void) {
    ...
    float u_roll  = -(K_roll_ang  * e_roll  + K_roll_rate  * gx_f + K_roll_int  * int_e_roll);
    float u_pitch = -(K_pitch_ang * e_pitch + K_pitch_rate * gy_f + K_pitch_int * int_e_pitch);
...
static void build_send_telem(uint32_t now_ms) {
    ...
    if (acklen == 26 && ack_buf[0] == PKT_CMD) {
        CmdPkt c;
        c.type              = ack_buf[0];
        c.cmd_seq           = ack_buf[1];
...
int main(void) {
    ...
    HAL_Delay(200);
    uart_print("BOOT V7 - LQI + AW + 250Hz PWM\r\n");
...
static void MX_TIM1_Init(void) {
    ...
    // CORRECCIÓN DE RETRASO: 3999 para 250 Hz PWM
...
static void MX_TIM2_Init(void) {
    ...
    htim2.Init.Period            = 4999;     // 1 MHz / 5000 = 200 Hz
