...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : STM32 LQI/LQG Multiplexer - V8.2 (Física V7 Restaurada)
  *
  * - Física base idéntica al V7 original (alpha_acc = 0.05).
  * - Telemetría continua incluso desarmado.
  * - Recepción de matrices Kalman dinámicas (0xB2).
  ******************************************************************************
  */
...
static uint8_t  current_ctrl_mode = 0; // 0=LQI, 1=LQG, 2=Fuzzy
...
// RESTAURADO EXACTAMENTE COMO EN V7 (Rápido y Responsivo)
...
/* ---------------------------------------------------------------
 * SETPOINTS, ACUMULADORES Y DEFAULTS
 * --------------------------------------------------------------- */
...
// Valores de tu calibración perfecta
...
typedef struct {
    ...
    uint8_t  imu_enable;        
    uint8_t  ping_id;           
    uint8_t  ctrl_mode;         // NUEVO BYTE (0=LQI, 1=LQG, 2=Fuzzy)
} CmdPkt;                        // 27 bytes
...
typedef char _check_CmdPkt_size  [sizeof(CmdPkt)   == 27 ? 1 : -1];
typedef char _check_TelemPkt_size[sizeof(TelemPkt) == 28 ? 1 : -1];
...
static void cal_step(uint32_t now_ms) {
    ...
    if (cal_count >= cal_samples_target) {
        mpu.Gx_offset = ((float)gxs / cal_count) / 131.0f;
        mpu.Gy_offset = ((float)gys / cal_count) / 131.0f;
        mpu.Gz_offset = ((float)gzs / cal_count) / 131.0f;
        mpu.Ax_offset = ((float)axs / cal_count) / 16384.0f;
        mpu.Ay_offset = ((float)ays / cal_count) / 16384.0f;
        mpu.Az_offset = (((float)azs / cal_count) / 16384.0f) - 1.0f;
        cal_busy = 0;
        uart_print("CAL DONE\r\n");
    }
}
...
/* ---------------------------------------------------------------
 * LOOP DE CONTROL @ 200 Hz
 * --------------------------------------------------------------- */
static void control_update(void) {
    if (!imu_enable || cal_busy) return;

    // --- FILTRADO FÍSICO EMA ---
    ax_f += alpha_acc  * (mpu.Ay - ax_f);
    ay_f += alpha_acc  * (-mpu.Ax - ay_f);  
    az_f += alpha_acc  * (mpu.Az - az_f);
    
    gx_f += alpha_gyro * (mpu.Gy - gx_f);
    gy_f += alpha_gyro * (-mpu.Gx - gy_f);  
    gz_f += alpha_gyro * (mpu.Gz - gz_f);

    float roll_acc  = atan2f(ay_f, az_f) * 57.29578f;
    float pitch_acc = atan2f(-ax_f, sqrtf(ay_f*ay_f + az_f*az_f)) * 57.29578f;

    // ========================================================
    // ETAPA 1: OBSERVADOR (SIEMPRE ACTIVO PARA LAS GRÁFICAS)
    // ========================================================
    float final_roll_est  = 0.0f;
    float final_pitch_est = 0.0f;
    float final_gx        = gx_f; 
    float final_gy        = gy_f;

    if (current_ctrl_mode == 0 || current_ctrl_mode == 2) {
        // --- MODO LQI / FUZZY --- (Filtro Complementario V7 Intacto)
        roll_deg  = 0.98f * (roll_deg  + gx_f * dt_control) + 0.02f * roll_acc;
        pitch_deg = 0.98f * (pitch_deg + gy_f * dt_control) + 0.02f * pitch_acc;
        
        final_roll_est  = roll_deg;
        final_pitch_est = pitch_deg;
        
        // Mantener Kalman sincronizado en caso de cambio brusco de modo
        x_hat_roll[0] = roll_deg; x_hat_roll[1] = gx_f;
        x_hat_pitch[0] = pitch_deg; x_hat_pitch[1] = gy_f;
    }
    else if (current_ctrl_mode == 1) {
        // --- MODO LQG --- (Filtro de Kalman)
        float x_pred_roll[2];
        ...
        float err_y_roll[2];
        err_y_roll[0] = roll_acc - x_pred_roll[0];
        err_y_roll[1] = gx_f - x_pred_roll[1];

        float err_y_pitch[2];
        err_y_pitch[0] = pitch_acc - x_pred_pitch[0];
        err_y_pitch[1] = gy_f - x_pred_pitch[1];
        ...
        roll_deg  = x_hat_roll[0];
        pitch_deg = x_hat_pitch[0];
        final_roll_est  = x_hat_roll[0];
        final_pitch_est = x_hat_pitch[0];
        final_gx        = x_hat_roll[1]; 
        final_gy        = x_hat_pitch[1];
    }

    // ========================================================
    // BLOQUEO DE SEGURIDAD (Si está desarmado, corta motores aquí)
    // ========================================================
    ...
    if (current_ctrl_mode == 0 || current_ctrl_mode == 1) {
        ...
        int_e_roll  += e_roll  * dt_control;
        int_e_pitch += e_pitch * dt_control;

        int_e_roll  = clamp_f(int_e_roll,  -int_limit, int_limit);
        int_e_pitch = clamp_f(int_e_pitch, -int_limit, int_limit);
    ...
    // ========================================================
    // ETAPA 3: MATRIZ DE MEZCLA
    // ========================================================
    u_roll  = clamp_f(u_roll,  -U_LIMIT, U_LIMIT);
    u_pitch = clamp_f(u_pitch, -U_LIMIT, U_LIMIT);

    int32_t thr = (int32_t)base_throttle;
    int32_t m1  = thr + (int32_t)u_roll  - (int32_t)u_pitch;
    int32_t m2  = thr - (int32_t)u_roll  - (int32_t)u_pitch;
    int32_t m3  = thr + (int32_t)u_roll  + (int32_t)u_pitch;
    int32_t m4  = thr - (int32_t)u_roll  + (int32_t)u_pitch;
    ...
    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, pwm_m1);
    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, pwm_m2);
    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_3, pwm_m3);
    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_4, pwm_m4);
}

void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim->Instance == TIM2) flag_200hz = 1;
}
...
static void handle_cmd(const CmdPkt *c) {
    ...
    if (c->flags & CMD_SET_AW) {
        int_limit = (float)c->aw_limit;
        uart_print("AW UPDATED\r\n");
    }
    ...
    if (c->flags & CMD_ARM) {
        if (!failsafe_active) {
            ...
            ax_f = mpu.Ay; ay_f = mpu.Ax; az_f = mpu.Az;
            gx_f = mpu.Gy; gy_f = mpu.Gx; gz_f = mpu.Gz;
            roll_deg  = atan2f(ay_f, az_f) * 57.29578f;
            pitch_deg = atan2f(-ax_f, sqrtf(ay_f*ay_f + az_f*az_f)) * 57.29578f;
            
            x_hat_roll[0] = roll_deg; x_hat_roll[1] = gx_f;
            x_hat_pitch[0] = pitch_deg; x_hat_pitch[1] = gy_f;

            motors_write_all(1000);  
            uart_print("ARMED (motors idle)\r\n");
        }
    }

    if ((c->flags & CMD_START_MOTORS) && armed) {
        motors_running = 1;
        uart_print("MOTORS RUNNING\r\n");
    }
}

static inline void buf_u8 (uint8_t *b, uint32_t *o, uint8_t  v) { b[(*o)++] = v; }
static inline void buf_u16(uint8_t *b, uint32_t *o, uint16_t v) {
    b[(*o)++] = (uint8_t)(v & 0xFF);
...
static void build_send_telem(uint32_t now_ms) {
    ...
    if (motors_running)  st |= ST_MOTORS_RUN;
    if (cal_busy)        st |= ST_CAL_BUSY;
    if (imu_enable)      st |= ST_IMU_EN;
    if (failsafe_active) st |= ST_FAILSAFE;

    buf_u8 (buf, &o, PKT_TELEM);                                        
    buf_u8 (buf, &o, tx_seq++);                                         
    buf_u32(buf, &o, now_ms);                                           
    buf_u16(buf, &o, (uint16_t)clamp_i16((int32_t)(roll_deg  * 10.0f))); 
    buf_u16(buf, &o, (uint16_t)clamp_i16((int32_t)(pitch_deg * 10.0f))); 
    buf_u16(buf, &o, (uint16_t)clamp_i16((int32_t)(gx_f * 10.0f)));      
    buf_u16(buf, &o, (uint16_t)clamp_i16((int32_t)(gy_f * 10.0f)));      
    buf_u16(buf, &o, (uint16_t)clamp_i16((int32_t)(gz_f * 10.0f)));      
    buf_u16(buf, &o, st);                                               
    buf_u16(buf, &o, (uint16_t)__HAL_TIM_GET_COMPARE(&htim1, TIM_CHANNEL_1)); 
    buf_u16(buf, &o, (uint16_t)__HAL_TIM_GET_COMPARE(&htim1, TIM_CHANNEL_2)); 
    buf_u16(buf, &o, (uint16_t)__HAL_TIM_GET_COMPARE(&htim1, TIM_CHANNEL_3)); 
    buf_u16(buf, &o, (uint16_t)__HAL_TIM_GET_COMPARE(&htim1, TIM_CHANNEL_4)); 
    buf_u8 (buf, &o, cmd_echo);                                         
    buf_u8 (buf, &o, ping_echo);                                        

    uint8_t ack_buf[32]; 
    uint8_t acklen = 0;
    (void)NRF24_WriteAndReadAck(
        &hspi1, &nrf,
        buf,     28,
        ack_buf, 32,
        &acklen, 15
    ...
    if (acklen == 27 && ack_buf[0] == PKT_CMD) {
        ...
        c.type              = ack_buf[0];
        c.cmd_seq           = ack_buf[1];
        c.flags             = (uint16_t)(ack_buf[2]  | (ack_buf[3]  << 8));
        c.thr_us            = (uint16_t)(ack_buf[4]  | (ack_buf[5]  << 8));
        c.sp_roll_x10       = (int16_t) (ack_buf[6]  | (ack_buf[7]  << 8));
        c.sp_pitch_x10      = (int16_t) (ack_buf[8]  | (ack_buf[9]  << 8));
        
        c.k_roll_ang_x1000  = (int16_t) (ack_buf[10] | (ack_buf[11] << 8));
        c.k_roll_rate_x1000 = (int16_t) (ack_buf[12] | (ack_buf[13] << 8));
        c.k_roll_int_x1000  = (int16_t) (ack_buf[14] | (ack_buf[15] << 8)); 
        
        c.k_pitch_ang_x1000 = (int16_t) (ack_buf[16] | (ack_buf[17] << 8));
        c.k_pitch_rate_x1000= (int16_t) (ack_buf[18] | (ack_buf[19] << 8));
        c.k_pitch_int_x1000 = (int16_t) (ack_buf[20] | (ack_buf[21] << 8)); 
        
        c.aw_limit          = (uint16_t)(ack_buf[22] | (ack_buf[23] << 8)); 
        c.imu_enable        = ack_buf[24];
        c.ping_id           = ack_buf[25];
        c.ctrl_mode         = ack_buf[26];
        handle_cmd(&c);
    }
    else if (acklen == 18 && ack_buf[0] == PKT_KALMAN) {
        L_roll[0][0] = (float)((int16_t)(ack_buf[2] | (ack_buf[3]<<8))) / 1000.0f;
        L_roll[0][1] = (float)((int16_t)(ack_buf[4] | (ack_buf[5]<<8))) / 1000.0f;
        L_roll[1][0] = (float)((int16_t)(ack_buf[6] | (ack_buf[7]<<8))) / 1000.0f;
        L_roll[1][1] = (float)((int16_t)(ack_buf[8] | (ack_buf[9]<<8))) / 1000.0f;

        L_pitch[0][0] = (float)((int16_t)(ack_buf[10] | (ack_buf[11]<<8))) / 1000.0f;
        L_pitch[0][1] = (float)((int16_t)(ack_buf[12] | (ack_buf[13]<<8))) / 1000.0f;
        L_pitch[1][0] = (float)((int16_t)(ack_buf[14] | (ack_buf[15]<<8))) / 1000.0f;
        L_pitch[1][1] = (float)((int16_t)(ack_buf[16] | (ack_buf[17]<<8))) / 1000.0f;
    }
}
...
int main(void) {
    ...
    HAL_Delay(200);
    uart_print("BOOT V8.2 - LQI/LQG MUX (Fisica V7 Restaurada)\r\n");
    ...
    nrf.ce_port  = GPIOB; nrf.ce_pin  = GPIO_PIN_0;
    nrf.csn_port = GPIOB; nrf.csn_pin = GPIO_PIN_1;
    if (NRF24_Init(&hspi1, &nrf) != NRF_OK) {
        uart_print("NRF FAIL\r\n");
        while (1) HAL_Delay(1000);
    }
    (void)NRF24_Configure_PTX(&hspi1, &nrf, RF_ADDR, RF_CH,
                               NRF_DATARATE_250K, NRF_PA_LOW);
    ...
    while (1) {
        ...
        if (cmd_echo != 0 && (now - last_cmd_ms) > FAILSAFE_MS) {
            if (!failsafe_active) {
                disarm_now(1);
            }
        }
...
/* ---------------------------------------------------------------
 * INICIALIZACIÓN DE HARDWARE (Mismo V7)
 * --------------------------------------------------------------- */
void SystemClock_Config(void) {
