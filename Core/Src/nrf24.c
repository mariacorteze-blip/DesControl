...
{
    uint8_t r = 0;
    if (HAL_SPI_TransmitReceive(hspi, &b, &r, 1, HAL_MAX_DELAY) != HAL_OK) return 0;
    return r;
}
...
/* NEW: ACTIVATE 0x73 to unlock FEATURE on many clones */
static void activate_features(SPI_HandleTypeDef *hspi, NRF24_t *nrf)
{
    uint8_t magic = 0x73;
    cmd_rw(hspi, nrf, CMD_ACTIVATE, &magic, NULL, 1);
}
...
NRF_Status NRF24_Init(SPI_HandleTypeDef *hspi, NRF24_t *nrf)
{
    ...
    // Minimal defaults
    write_reg(hspi, nrf, REG_EN_AA,     0x01); // pipe0
    write_reg(hspi, nrf, REG_EN_RXADDR, 0x01); // pipe0

    // CRC16, power down for now
...
{
    // unlock feature first (safe to call always)
    activate_features(hspi, nrf);

    uint8_t f = read_reg(hspi, nrf, REG_FEATURE);
    if(en) f |= FEATURE_EN_DPL;
    else   f &= (uint8_t)~FEATURE_EN_DPL;
    write_reg(hspi, nrf, REG_FEATURE, f);

    write_reg(hspi, nrf, REG_DYNPD, en ? 0x01 : 0x00); // pipe0
}
...
static void enable_ack_payload(SPI_HandleTypeDef *hspi, NRF24_t *nrf, bool en)
{
    ...
    else   f &= (uint8_t)~FEATURE_EN_ACK_PAY;
    write_reg(hspi, nrf, REG_FEATURE, f);
}
...
static void open_writing_pipe(SPI_HandleTypeDef *hspi, NRF24_t *nrf, const uint8_t addr[5])
{
    ...
    write_reg_buf(hspi, nrf, REG_RX_ADDR_P0, addr, 5); // AutoACK needs this
}
                               ...
                               NRF_PaLevel pa)
{
    ...
    // Clear any stale RX (important for ACK payload usage)
    NRF24_FlushRX(hspi, nrf);
    NRF24_ClearIRQs(hspi, nrf);
...
/* Better check: ACK payload -> data is in RX FIFO (RX_EMPTY=0) */
bool NRF24_AckPayloadAvailable(SPI_HandleTypeDef *hspi, NRF24_t *nrf)
{
    (void)hspi;
    uint8_t fifo = read_reg(hspi, nrf, REG_FIFO_STATUS);
    return (fifo & FIFO_RX_EMPTY) == 0;
}
...
/* NEW: One-shot TX and read ACK payload if present */
NRF_Status NRF24_WriteAndReadAck(SPI_HandleTypeDef *hspi, NRF24_t *nrf,
                                 ...
                                 uint32_t timeout_ms)
{
    if (acklen_out) *acklen_out = 0;

    NRF_Status s = NRF24_Write(hspi, nrf, txbuf, txlen, timeout_ms);
    if (s != NRF_OK) return s;

    // If PRX put an ACK payload, it should now be in RX FIFO.
    if (ackbuf && ackmax > 0 && NRF24_AckPayloadAvailable(hspi, nrf)) {
        uint8_t n = NRF24_ReadAckPayload(hspi, nrf, ackbuf, ackmax);
        if (acklen_out) *acklen_out = n;
    }
    return NRF_OK;
}
